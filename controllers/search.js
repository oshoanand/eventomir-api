...
export const searchPerformers = async (req, res) => {
  try {
    ...
    // 1. Generate a deterministic Cache Key based on search parameters.
    // Sorting the keys ensures URL param order doesn't create duplicate cache entries.
    ...
    // Create string like "accountType=all&city=Moscow&page=1..."
    const cacheParams = new URLSearchParams(sortedQuery).toString() || "all";

    // 2. Define the database query as an async function
    // This will ONLY run if there is a Cache Miss (or if Redis is down)
    const dbQuery = async () => {
      const where = {
        role: "performer",
        status: "active", // Only show active performers
      };
      ...
      if (onlyVip === "true") {
        // Match the exact string casing used in your database
        where.subscription_plan_id = "PREMIUM";
      }
      ...
      const [users, total] = await prisma.$transaction([
        prisma.user.findMany({
          ...
          orderBy: {
            created_at: "desc", // Newest performers first
          },
        }),
      ...
      if (priceMin || priceMax) {
        filteredUsers = users.filter((user) => {
          if (!user.price_range || user.price_range.length === 0) return false;
          const minP = user.price_range[0];
          const maxP = user.price_range[1] || user.price_range[0];
      ...
      const formattedItems = filteredUsers.map((user) => ({
        id: user.id,
        name: user.name,
        city: user.city,
        ...
        isVip: user.subscription_plan_id === "PREMIUM",
        parentAgencyName: user.parentAgency?.name || null,
      }));

      return {
        items: formattedItems,
        total: priceMin || priceMax ? filteredUsers.length : total,
        page: parseInt(page),
        pageSize: take,
      };
    };

    // 3. Execute through the Redis Cache wrapper
    // Resource: "search:performers"
    // ID: cacheParams (e.g. "page=1&city=Moscow")
