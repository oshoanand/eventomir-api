...
router.post("/register-performer", async (req, res) => {
  try {
    ...
    // 🚨 FIX: Changed findUnique to findFirst to avoid crashes on non-unique fields
    const freePlan = await prisma.subscriptionPlan.findFirst({
      where: { tier: "FREE" },
    });
      ...
      data: {
        ...
        ...(freePlan && {
          subscription: {
            create: {
              planId: freePlan.id,
              status: "ACTIVE", // 🚨 FIX: Updated from isActive: true
              pricePaid: 0, // 🚨 FIX: Added mandatory pricePaid field
            },
          },
        }),
